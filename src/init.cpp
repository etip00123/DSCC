...
BaseCache* BuildCacheBank(Config& config, const string& prefix, g_string& name, uint32_t bankSize, bool isTerminal, uint32_t domain) {
    ...
    uint32_t superBlockSize = (arrayType == "SCC" || arrayType == "DSCC") ? config.get<uint32_t>(prefix + "array.superblock", 8) : 1;

    // If using cache compression techniques, the array is multiplied by the size of the super block
    // So that the compressed cache can be simulated without messing with all current ZSim code
    if (arrayType == "SCC" || arrayType == "DSCC"){
    ...
    if (arrayType == "SetAssoc") {
        numHashes = 1;
    } else if (arrayType == "Z" || arrayType == "SCC" || arrayType == "DSCC") {
        numHashes = ways;
        assert(ways > 1);
    ...
    if (numHashes) {
        if (hashType == "None") {
            if (arrayType == "Z" || arrayType == "SCC" || arrayType == "DSCC") {
                panic("ZCaches, Skewed Compressed Caches and Dynamically Skewed Compressed Caches must be hashed!"); //double check for stupid user
            }
            assert(numHashes == 1);
            hf = new IdHashFamily;
        } else if (hashType == "H3") {
    ...
    CacheCompressor* compressor = nullptr;
    if (arrayType == "SCC" || arrayType == "DSCC") {
        string compressorType = config.get<const char*>(prefix + "array.compressor", "CPack");
        if (compressorType == "CPack"){
            compressor = new CPack(16);
        } else {
            panic("%s: Invalid cache compressor type %s", name.c_str(), compressorType.c_str());
        }
    }
    ...
    } else if (arrayType == "SCC") {
        array = new SCCArray(numLines, ways, superBlockSize, rp, hf, compressor);
    } else if (arrayType == "DSCC") {
        array = new DSCCArray(numLines, ways, superBlockSize, rp, hf, compressor);
    } else if (arrayType == "Z") {
    ...
    if (!isTerminal) {
        ...
        if ((arrayType != "SetAssoc" && arrayType != "SCC" && arrayType != "DSCC") || hashType != "None" || replType != "LRU") panic("Invalid FilterCache config %s", name.c_str());
