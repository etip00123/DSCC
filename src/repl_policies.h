...
class ReplPolicy : public GlobAlloc {
    protected:
        ...
        CacheArray* cacheArray; // Cache array, used to figure out the superblock of the candidates

    public:
        ReplPolicy() : cc(nullptr), cacheArray(nullptr) {}

        virtual void setCC(CC* _cc) {cc = _cc;}
        virtual void setCacheArray(CacheArray* _cacheArray) {cacheArray = _cacheArray;}
        ...
        virtual uint32_t rankCands(const MemReq* req, DSCCCands cands) = 0;
...
#define DECL_RANK_BINDINGS DECL_RANK_BINDING(SetAssocCands); DECL_RANK_BINDING(ZCands); DECL_RANK_BINDING(DSCCCands);
...
class LRUReplPolicy : public ReplPolicy {
    ...
    private:
        inline uint64_t score(const MemReq* req, uint32_t id) { //higher is least evictable
            //array[id] < timestamp always, so this prioritizes by:
            // (1) valid (if not valid, it's 0)
            // (2) sharers, and
            // (3) timestamp
            return (cacheArray? cacheArray->matchArray(req->lineAddr, id)*!cc->isValid(id) : 0)*timestamp + (sharersAware? cc->numSharers(id) : 0)*timestamp + array[id]*cc->isValid(id);
        }
};
