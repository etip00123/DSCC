...
uint64_t Cache::access(MemReq& req) {
    ...
    if (likely(!skipAccess)) {
        ...
        if (lineId == -1 && cc->shouldAllocate(req)) {
            ...
            std::vector<Address> wbLineAddr;
            std::vector<int32_t> evictId;

            lineId = array->preinsert(req.lineAddr, &req, &wbLineAddr, &evictId); //find the lineId to replace

            std::vector<int32_t>::iterator itId = evictId.begin();
            for (std::vector<Address>::iterator itAddr = wbLineAddr.begin(); itAddr != wbLineAddr.end(); ++itAddr, ++itId){
              trace(Cache, "[%s] Evicting 0x%lx", name.c_str(), *itAddr);

              //Evictions are not in the critical path in any sane implementation -- we do not include their delays
              //NOTE: We might be "evicting" an invalid line for all we know. Coherence controllers will know what to do
              cc->processEviction(req, *itAddr, *itId, respCycle); //1. if needed, send invalidates/downgrades to lower level
